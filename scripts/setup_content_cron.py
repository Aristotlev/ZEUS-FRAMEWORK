...
def _build_jobs(niche: List[str]):
    ...
    breaking_news = (
        ...
        "headlines (one OpenRouter call each), and AUTO-PUBLISHES at most ONE "
        "item per pass (highest score wins, threshold >= 0.85) via the "
        "ARTICLE pipeline (short-form text-only -> Publer fan-out + Substack "
        "Note). Losers above threshold are marked seen so they don't re-score. "
        "Hard ceiling: 6 posts/hr (cron fires every 10 min). The script's "
        "own run() call into pipeline_test.py owns Notion archive, cost "
        "ledger, and email rollup for each shipped item.\n\n"
        f"COMMAND (this is the entire task):\n"
    ...
    # watcher_supervisor.sh and self-respawns. carousel_slot and
